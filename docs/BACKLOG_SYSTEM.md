...
來源專案實作：`docs/BACKLOG.md`（索引）＋ `docs/backlog/<topic>.md`（分題）。
...
| 路徑 | 職責 | 何時讀 |
| --- | --- | --- |
| `docs/BACKLOG.md` | 工程主題目錄；狀態＋一句摘要 | 「有咩未做」／list backlog |
| `docs/backlog/<topic>.md` | 範圍、結論、待決、進度、相關連結 | 做該主題時 |
| `docs/plans/…` | 現行實作步驟、設計稿、驗收 checklist | 僅 `in_progress` 且要跟步驟；**只開現行計劃** |
| `docs/audits/…` | 已完成調查結果 | 對對抗、查「點解咁決定」、或用戶點名 |
| 規範／營運／構想（各自檔） | 政策、UI 規範、`FUTURE_*.md` 等 | 索引用「想找…」表指過去；**唔當待辦列** |

---

## 3. 索引設計（`docs/BACKLOG.md`）
...
> 問未做 → `docs/BACKLOG.md`「進行中／未完成」。做主題 → 該列 `docs/backlog/<topic>.md`（＋現行 `docs/plans/`）。

---

## 7. 新專案最小落地步驟

1. 建立 `docs/BACKLOG.md`（用下方骨架）。  
2. 建立 `docs/backlog/`（可先空）。  
3. （可選）預留 `docs/plans/`、`docs/audits/`。  
...
- 新主題：加列到「進行中／未完成」＋可選 `docs/backlog/<topic>.md`
- 開始做：狀態改 `in_progress`；有計劃連 `docs/plans/…`

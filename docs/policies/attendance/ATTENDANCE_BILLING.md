...
**系統現況：** `2627` 點名紙入場＝該堂到課宣告 ∪ 已確認收款試堂 ∪ 請假調堂。**不准手動加名**（2026-08-31 取消 P6）。`26SM` **永遠**沿用暑期舊路徑（期數過濾），唔走已繳堂數池／到課宣告；補回查重標記不得清除。
...
| 私人課程、試堂、功課輔導班 | 否（跟該班） |
| 混級專科班 | 否（跟該班） |
| 暑期 `*SM`（含 `26SM`） | 唔走此池。**`26SM` 永遠舊路徑**（2026-08-31 拍板：唔日落、唔遷池） |
...
`2627` 點名紙上有名，只經：該堂到課宣告（報讀自動出）、已確認收款試堂、或請假調堂掛入該堂。**不准**在點名紙上手動加名，亦不准直接寫入出席紀錄繞過上述入口。名單缺口用單堂報讀或調堂。

## 暑期 `26SM`（永遠舊路徑）

`26SM` **不准**改用 `2627` 已繳堂數／到課宣告模型，亦不准把舊報讀補鑄已繳堂數。點名紙仍按報讀期數（第一期／第二期／兩期全報）及單堂綁定過濾。後續 `*SM`（如 `27SM`）是否改用常規模型，另議。

已安排補回的取消堂，系統會留下對應備註（`makeup_of=`）；**不准清除**，否則補堂可重複開、暑期歷史點名紙會缺人。`2627` 點名紙入場不靠此備註。

...
班別／排程／功能愈多時，**最容易犯錯的是把「任教老師」與「實際授課老師」混為一談**。本檔記錄正確做法與已知行政風險，避免同類問題重演。
...
| 層級 | 欄位 | 含義 |
| --- | --- | --- |
| 班別 | `classes.teacher_id` | **任教老師**（一班一個；「我的班別」等依此） |
| 排程堂次 | `schedules.teacher_id` | **實際授課老師**（可與任教老師不同） |
| 代堂標記 | `schedules.original_teacher_id` | 指派代堂前的原任老師；有值＝此堂為代堂 |

新建排程時會從班別任教老師複製 `teacher_id`。之後若有人代上，應改**該堂排程**，不要更換班別任教老師。
...
3. DB：班別首次指定／更換任教老師時，回填該班仍空白且非代堂的排程（不覆寫已有老師或代堂列）。

點名（`attendance_details`）掛 `schedule_id`／`class_id`，**不另存老師**；畫面上「誰上這堂」是讀排程的 `teacher_id`（缺則回退班別任教老師）。寫入點名仍要求當日 `schedules.teacher_id`＝登入老師。

```mermaid
flowchart LR
  C[classes.teacher_id<br/>任教老師] -->|新建排程時複製| S[schedules.teacher_id<br/>實際授課]
  S -->|指派代堂| S2[teacher_id = 代堂老師<br/>original_teacher_id = 原任]
...
### 偶發／輪流代堂（同一班、不同日子不同老師）

1. **班別任教老師維持不變**（例如 Kenneth）。
2. 代堂日在排程用「**指派代堂**／更改代堂」寫上實際授課老師（例如 Liam）。
3. 歷史補錄亦同：每一堂排程各自設正確的當日老師；可用代堂或建立時直接指定 `schedules.teacher_id`。
4. **不要**為了偶發代堂去改 `classes.teacher_id`（會影響「我的班別」、報讀報表的任教老師歸屬，並與已過去堂次語意錯位）。
...
**入系統原則**：開一班（任教老師可訂為較常上者或行政指定的常任老師），每堂排程的 `teacher_id`＝上表當日老師；由其他老師授課的日子用「指派代堂」標出，並一併完成點名。
...
| 排程名單 | 任教老師、實際授課老師與原任透過 `get_teacher_schedule_roster_context(schedule_id[])` 讀取該堂最小名單資料；不因此取得整班 `students` 權限 |
...
| 風險 | 說明 | 應對 |
| --- | --- | --- |
| 老師堂數／出勤歸屬偏向任教老師 | 部分查詢曾偏 `classes.teacher_id` | 算薪／「誰上了幾堂」必須以 **`schedules.teacher_id`** 為準；老師詳情出勤已改（2026-08） |
| 事後取消代堂改寫歷史 | 點名列**沒有凍結當日老師** | **已點名禁止取消代堂**（`clearScheduleSubstitute`＋UI）；要改用「更改代堂」 |
| 「我的班別」不含代堂班 | 代堂老師不會在班別列表擁有該班，只透過當日排程進入 | 預期行為；額外提醒暫緩 |
...
> **班別記任教老師，排程記實際授課老師；偶發代堂只改排程，算堂數看排程老師，已點名勿清代堂。**

...
### 2026-08-31 — 非本班上紙先問是否同一資料流；專科池不要亂隔離
- **情境**：非常規排程拍板（試堂／單堂報讀／混合補堂非原班生）。
- **錯在邊**：把三條當成三套工程；又曾建議單堂不入專科池以免牽動逾期罰款。3.2 例子把「同一班加堂」講得像 a／b 兩班。
- **正確做法**：非本班就讀生上該堂點名紙＝一條資料流，標籤與扣袋才分叉。`2627` 專科池是學生×學年×**同一級**科目共用；試堂獨立。加堂挑選（已就讀生）是另一條。
- **若已升格**：未升格（見分題 `nonstandard-scheduling.md` 結論「同一資料流」）

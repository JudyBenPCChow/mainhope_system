...
| 狀態 | 優先 | 主題 | 摘要 | 詳情 |
| --- | --- | --- | --- | --- |
| in_progress | 高 | 技術債／工程硬化 | **P0-1** production RLS 已套（2026-08-19；`authz_version=10`）。JWT 模擬煙霧已跑；真人畫面未走。側欄唔改（IA1）。**P0-2 前端實作已清**（同日）：service／畫面 `can()`／收件匣 actor RPC／敏感路由只靠 `RequireCapabilities`（頁內角色門已拆）／計糧、讓房及老師 scope 全部改讀 Auth profile。Supabase Auth JWT 原生 `session_id` 已核實；SQL 模擬同一帳戶兩個 session 可分別戴 teacher／manager（測試列已刪）。餘可選真人雙裝置畫面。 | [tech-debt-hardening.md](./topics/tech-debt-hardening.md) · [稽核](./audits/2026-08-14-tech-debt-review.md) · [P0-1 重評](./topics/p0-1-authorization-redesign.md) · [功能×角色](./topics/p0-1-authz-feature-roles.md) |
...
| open | 高 | 2627 九月常規時間表 | 2026-08-19 **ver. 3.3** 候選（63 班；方案／老師附件／周時間表分冊，md＋Word／PDF）；3.2 及更早檔案保留；待簽收；prod 仍 0 班 | [2627-september-timetable.md](./topics/2627-september-timetable.md) · [方案 PDF](../year/2627/timetable/versions/v3.3/2627_timetable_scheme_v3.3.pdf) · [老師 PDF](../year/2627/timetable/versions/v3.3/2627_timetable_teachers_week_v3.3.pdf) · [周時間表 PDF](../year/2627/timetable/versions/v3.3/2627_timetable_weekly_v3.3.pdf) · [排課規則](../policies/scheduling/SCHEDULING_RULES.md) |

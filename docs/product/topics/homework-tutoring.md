...
| 欄位 | 值 |
| --- | --- |
| 狀態 | `in_progress`（**H11 2026-08-24 通過**；波次 1 正式側欄） |
| 優先 | 中 |
| 範圍 | 一班制、按月繳費報讀、課室佔用、導師月度編更、獨立功輔校曆；三角色畫面 |
| 不含 | 計糧功輔時薪（交計糧）；暑期功輔產品化；學生點名紙；**每日功課進度／指示檔正文（留 Notion）**；宣傳物料；專科式請假／補堂／扣堂 |
| 索引 | [`BACKLOG.md`](../BACKLOG.md) |
| 盤點／定案 | 2026-08-01；待決回覆吸納 2026-08-11 |
| 上次更新 | 2026-08-24（**H11 通過**；波次 1 正式側欄） |
...
5. **2026-08-23–24 H11 沙盒修訂**：老師報更已填淡橙、去掉放假日分頁；行政概覽本日學生＋中／小學上下卡片；報讀學生＋學部篩選；**副頁改側欄清單預覽（唔用頁內標籤）**；**報讀唔另開表**（走學生管理 → 報讀班別）；**管理層剔選專科老師側欄入口**。
6. **2026-08-24 H11 通過**；開正式實作 plan。

### 下一步

1. 波次 1：正式側欄＋老師入口持久化（畫面暫假資料）。
2. 其後見實作 plan 波次 2–5（班型／報讀、月費、編更、2627 §7）。

### 尚未開始：正式產品其餘波次

- Schema／RLS（除老師側欄旗標）、正式報讀月費、課室編更寫入（放假日已入 `homework_tutoring_calendar_closures`）
- 計糧功輔時薪
...
| --- | --- |
| 科目 | `subjects` 已有 `HWK` |
| 班型 | `class_kind` 僅 `group`／`private`（正式功輔班仍用 `students`＋`enrollments`，**唔另建報讀表**；H11 後先加班型／班） |
| 排課規則 | §4 已：**預設 17D**、可調、分界 **17:00**；末節讓房**不做** |
| 計糧 | 功輔暫不開工 |
| 產品功能 | **波次 1**：系統側欄一級「功課輔導」（打開見二級）；編更／報讀／月費暫假資料。老師入口旗標已入 `teachers.homework_tutoring_nav` |
...
2. 排課 §4 已跟；讓房**不做**。
3. UI 以 v2 為準；正式實作見 [`2026-08-24-homework-tutoring-implementation.md`](../plans/2026-08-24-homework-tutoring-implementation.md)。

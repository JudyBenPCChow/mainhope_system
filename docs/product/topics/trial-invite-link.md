...
- **雙閘**：① 職員核准 → 建 `trial_sessions`（帶入產生連結時選定的免費／半價／原價，審核可改）；② 收款確認已收款 → 上點名紙

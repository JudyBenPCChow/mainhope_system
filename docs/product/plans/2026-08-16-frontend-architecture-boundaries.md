...
| --- | --- |
| 日期 | 2026-08-16 |
| 狀態 | 方案已審＋顧問覆核已吸收；**波次 1–4 已落地**；本題可關閉（唔聲稱完整單向分層） |
| 讀者 | 實作 agent（本檔自洽；唔使先讀完技術債全文） |
...
| 對抗 | [`2026-08-16-frontend-architecture-boundaries-adversarial.md`](../audits/2026-08-16-frontend-architecture-boundaries-adversarial.md)（落地後遺；紅線已併入 §4.4） |
| 顧問覆核 | [`2026-08-16-frontend-architecture-boundaries-consultant-review.md`](../audits/2026-08-16-frontend-architecture-boundaries-consultant-review.md)（採納項已寫入本檔；與本檔衝突以本檔為準） |
| 索引 | [`BACKLOG.md`](../BACKLOG.md) |
| 性質 | **資料存取邊界**＋營運數字可信；**不改**學費、報讀、點名等營運規則。本期**唔聲稱**完成完整 `component → service → lib` 單向分層 |

本檔要回答：點解有呢題、依家傷喺邊、原方案有咩問題、改善後做咩、點先算完。細節證據喺分題。顧問覆核已吸收；開工以 §4 為準。
...
原方案把「拆 God files」同「修靜默 0」捆成四波重構。第一性審核後：**先收回直打 DB＋修 KPI 謊言（本期可關閉）；拆畫面只喺切片能自己擁有狀態、或第二個畫面已共用時先做。** 唔為行數搬家。顧問覆核後再收窄：本期鎖嘅係**資料存取邊界**（UI／page／lib 唔打表），唔係完整單向分層；失敗單位係**依賴群組**，唔係逐張卡各自「—」。

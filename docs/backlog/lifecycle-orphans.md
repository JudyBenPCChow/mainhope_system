...
| 欄位 | 值 |
| --- | --- |
| 狀態 | `in_progress`（**階段 A 完成**：A1＋A2a＋A2b O2 已落 code；**B／C 未做**） |
| 優先 | 高 |
...
| 相關政策 | [`ATTENDANCE_BILLING.md`](../ATTENDANCE_BILLING.md)、[`LEAVE_MAKEUP_CONSECUTIVE.md`](../manual/LEAVE_MAKEUP_CONSECUTIVE.md) §6 |
| 更新日期 | 2026-08-01 |
...
| 高 | 手誤清除報讀 | 出席可用 O2 刪；報讀變更掃出席仍待 **O4** |
| 高 | 取消／硬刪排程 | 仍待 **O3／O5** |
| 中 | 軟退讀／改期數 | 仍待 **O0／O4** |
...
| 做法 | 明學採用 |
| --- | --- |
| PowerSchool：會產孤兒則擋／Confirm | **已落**：取消補堂／清調堂／disposition／試堂 |
...
| ID | 階段 | 狀態 | 項目 | 說明 | 建議 |
| --- | --- | --- | --- | --- | --- |
| O0 | B | 未做 | 可見性 | 標「資格已結束（歷史出席仍計）」 | S02／S14 |
| O3 | B | 未做 | 軟取消排程對齊 | 調堂改回待安排；試堂提示／取消 | S12 |
| O4 | C | 未做 | 退讀／清報讀掃出席 | 變更前 Confirm（退讀預設保留） | S11／S13／S14 |
| O5 | C | 未做 | 對帳健康檢查 | 無應到資格（含 `schedule_id` null）唯讀＋一鍵清 | 可後做 |

## 行政邊緣模擬（2026-07-31；狀態已對齊 A 完成）

| 模擬 ID | 個案 | 現況判定 | 對應 |
| --- | --- | --- | --- |
| S01 | 林藝涵型：取消已點名補堂 | **可完成**（A1） | O1 |
| S02 | 真有來補堂、誤取消 | 可完成但易錯；保留後難發現 → **O0** | O0 |
| S03 | disposition→錄影等 | **可完成**（A2a O1-type） | O1-type |
| S11 | 試堂取消／改期出席 | **試堂出席可完成**（O1t）；退讀等仍待 O4 | O1t；O4 |
| S12 | 軟取消整堂 | 半完成 | O3 |
| S13 | purge 報讀後出席 | **可用 O2 單列刪收尾**；報讀變更掃出席仍待 O4 | O2；O4 |
| S14 | 退讀後真上課保留 | 可完成但易錯 | O0；O4 |
| S15 | 一併刪＋同時重存點名 | **已防寫回**（O1-rollcall） | O1-rollcall |
| S20 | 老師精靈撞已點名補堂 | 可完成（轉行政） | O6 SOP |

**接手優先（本主題內）：** B（O0＋O3）→ C（O4＋O5）。

## 個案應急（林藝涵）

- 補堂孤兒敘事：請假已刪；庫內 `2026-07-25` 剩「現場」且該班有報讀 → **毋須刪**。  
- 日後同類應急：優先用 **學生詳情／出席紀錄「刪除」**（O2）；無 UI 權限時仍可用 SQL runbook（見 [`LIFECYCLE_ORPHAN_CLEANUP_RUNBOOK.md`](../manual/LIFECYCLE_ORPHAN_CLEANUP_RUNBOOK.md)）。

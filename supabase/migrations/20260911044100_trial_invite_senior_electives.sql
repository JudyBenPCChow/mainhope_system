-- 試堂邀請：中四或以上先填選修；目錄帶科目分類；申請保存選修碼

begin;

alter table public.trial_invite_requests
  add column if not exists elected_subject_codes text[] not null default '{}'::text[];

comment on column public.trial_invite_requests.elected_subject_codes is
  '高中生公開頁多選之選修科目 code（subjects.code）；主科不必列入。';

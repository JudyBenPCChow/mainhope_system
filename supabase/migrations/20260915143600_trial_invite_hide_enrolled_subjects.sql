-- 公開試堂目錄：本學年已報讀的科目不再出現（專科／功輔各自比對）
...
comment on function public.trial_invite_subject_open_for_student(uuid, uuid) is
  '試堂邀請公開目錄：學生在本學年（is_current）就讀中已報讀同一產品線同一科目則不顯示。';

revoke all on function public.trial_invite_subject_open_for_student(uuid, uuid) from public;
revoke all on function public.trial_invite_subject_open_for_student(uuid, uuid) from anon;
grant execute on function public.trial_invite_subject_open_for_student(uuid, uuid) to authenticated;
  ...
  if r.status = 'open' then
    ...
    from (
      ...
      where public.trial_invite_class_matches_student(c.id, r.student_id)
        ...
        and public.trial_invite_subject_open_for_student(c.id, r.student_id)
        and exists (
          select 1

-- 排程限定名單：代堂老師只透過獲授權 schedule_id 取得點名所需的最小學生資料。
-- 不擴闊 students / student_class_enrollments 的基礎 RLS，避免代過一堂便永久讀取整班。
...
revoke all on function public.get_teacher_schedule_roster_context(uuid[]) from public, anon;
grant execute on function public.get_teacher_schedule_roster_context(uuid[]) to authenticated;

comment on function public.get_teacher_schedule_roster_context(uuid[]) is
  'Returns minimal roster context for explicitly authorized schedules; used by owner/substitute roll call without broad students RLS.';

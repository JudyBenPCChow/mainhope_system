-- Roster RPC：職員列舉加 finance（is_mgmt_staff 已含 finance；授權計數自動通）
-- 套用：npm run db:apply -- supabase/migrations/20260804112355_roster_rpc_finance_role.sql
--
-- rollback（手動）：將下方 gate 改回
...
comment on function public.get_teacher_schedule_roster_context(uuid[]) is
  'Returns minimal roster context for authorized schedules; mgmt staff (admin／manager／finance／alien) or teacher with schedule access.';

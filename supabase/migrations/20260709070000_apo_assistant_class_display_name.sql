-- 明學IT狗：老師班別／今日點名查詢加入 course_name（班名）
...
begin
  ...
  select coalesce(jsonb_agg(to_jsonb(c) order by c.day_of_week, c.time_slot, c.class_name), '[]'::jsonb)
  into v_classes
  from (
    select
      cl.id as class_id,
      cl.subject,
      coalesce(nullif(trim(cr.course_name), ''), cl.subject) as class_name,
      cr.course_name,
      cl.course_code_full,
      cl.section_code,
      cl.day_of_week,
    ...
    left join public.courses cr on cr.id = cl.course_id
    where cl.teacher_id = p_teacher_id
    order by cl.day_of_week nulls last, cl.time_slot nulls last, class_name
  ) c;
...
do $$
begin
  execute 'revoke all on function public.apo_assistant_teacher_classes(uuid,text,uuid) from public';
  execute 'revoke all on function public.apo_assistant_teacher_classes(uuid,text,uuid) from anon';
  execute 'revoke all on function public.apo_assistant_teacher_classes(uuid,text,uuid) from authenticated';
  execute 'grant execute on function public.apo_assistant_teacher_classes(uuid,text,uuid) to service_role';

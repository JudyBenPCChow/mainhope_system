...
function isMoneyTab(tab: StudentDetailTabId): boolean {
 return tab === "payments" || tab === "tuitionChase"
}

/** 無權限或無效 tab → 基本資料。能力尚未載入時，暫不把繳費／學費追收校正掉。 */
export function parseStudentDetailTab(
 ...
 if (isMoneyTab(t) && opts.capsReady && !opts.canViewMoney) return "basic"
 return t
}

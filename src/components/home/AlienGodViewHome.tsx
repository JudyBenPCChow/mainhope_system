...
export function AlienGodViewHome() {
  ...
  return (
    <div className="space-y-8 p-4 md:p-6 lg:space-y-10">
      <header className="flex flex-wrap items-end justify-between gap-4 border-b border-violet-200/60 pb-6">
        <div>
          <p className="text-sm font-medium uppercase tracking-wide text-violet-800/90">外星人 · 上帝視角</p>
          <h1 className="mt-2 flex flex-wrap items-center gap-2 text-3xl font-bold tracking-tight text-foreground md:text-4xl">
            <Orbit className="h-9 w-9 text-violet-600" aria-hidden />
        ...
        <div className="flex flex-wrap gap-2">
          <Button type="button" variant="default" size="sm" className="gap-2 bg-violet-700 hover:bg-violet-800" asChild>
            <Link to="/SystemLogs">
          ...
          {loading ? (
            ...
            <table className="w-full min-w-[640px] text-left text-sm">
              <thead className="border-b border-border bg-muted/40 text-muted-foreground">
                <tr>
                  <th className="px-4 py-3 font-medium md:px-5">時間</th>
                  <th className="px-4 py-3 font-medium">使用者</th>
                  <th className="px-4 py-3 font-medium">角色</th>
                  <th className="px-4 py-3 font-medium">操作</th>
                  <th className="px-4 py-3 font-medium md:pr-5">路徑／備註</th>
                </tr>
              </thead>
            ...
            <table className="w-full min-w-[640px] text-left text-sm">
              <thead className="border-b border-border bg-muted/40 text-muted-foreground">
                <tr>
                  ...
                  <th className="px-4 py-3 font-medium md:pr-5">狀態／詳情</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border">
                {errorRows.map((r) => (

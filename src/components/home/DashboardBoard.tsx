      ...
      <section className="flex min-w-0 flex-1 flex-col gap-5 lg:w-[70%]" aria-label="今日概況">
        ...
        <div className="rounded-xl border border-border/80 bg-card/90 p-5 shadow-sm md:p-6">
          <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
            <h2 className="flex flex-wrap items-center gap-2 text-base font-semibold text-foreground md:text-lg">
              <ListTodo className="h-5 w-5 shrink-0 text-violet-600" aria-hidden />
              待辦事項
              <span className="rounded-full bg-muted px-2.5 py-0.5 text-sm font-medium text-muted-foreground">
            ...
            >
              <ul className="space-y-2">
                {todosToday.map((t) => (
                  ...
                  >
                    <CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0 text-violet-500/70" aria-hidden />
        ...
        >
          ...
          {loading ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[320px] border-collapse text-center text-xs md:text-sm">
                <thead>
                  <tr>
                    ...
                    {roomVacancy.map((col) => (
                      <th
                        key={col.roomId}
                        className="max-w-[5.5rem] border border-border/60 bg-muted/40 px-1 py-2 font-medium leading-tight text-foreground"
                        title={col.roomName}
                      >

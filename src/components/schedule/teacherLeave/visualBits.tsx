...
export const TEACHER_LEAVE_STEP_LABELS = [
  "選老師與日期",
  "逐堂決策",
  "確認執行",
  "完成摘要",
] as const
export type TeacherLeaveWizardStep = 1 | 2 | 3 | 4
...
export function WizardStepRail({
  ...
  step: TeacherLeaveWizardStep | "done"
  maxReached: TeacherLeaveWizardStep
  onGo: (n: TeacherLeaveWizardStep) => void
}) {
  return (
    <ol className="grid gap-2 sm:grid-cols-4" aria-label="老師請假處理步驟">
      {TEACHER_LEAVE_STEP_LABELS.map((label, i) => {
        const n = (i + 1) as TeacherLeaveWizardStep

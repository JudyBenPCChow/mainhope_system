    ...
    {sheetLoading ? (
     ...
     <StaggerList as="div" className="space-y-3">
      {students.map((row, idx) => (
       <StaggerItem key={row.studentId} as="article" className="rounded-xl border border-border bg-background p-4 shadow-sm">
     ...
     <div className="overflow-x-auto">
      <table className="w-full min-w-[720px] table-fixed border-collapse text-sm">
       ...
       <StaggerList as="tbody">
        {students.map((row, idx) => (
         <StaggerItem key={row.studentId} as="tr" className="border-b border-border last:border-0">

...
import { contactUpdatePublicUrl } from "@/services/contactUpdateQueries"
...
export type ContactUpdatePrintSlip = {
  id: string
  full_name: string
  student_code: string
  grade_label: string
...
export function ContactUpdatePrintSlips({
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      for (const r of rows) {
        const url = contactUpdatePublicUrl(r.token)
        try {
  ...
  return (
    <div className="cu-print-root" role="dialog" aria-label="學生資料更新頁打印預覽">
      <div className="cu-print-toolbar">
        <p className="text-sm text-muted-foreground">打印預覽 · {rows.length} 頁（一人一張）</p>

...
function classTableHtml(c: MockClassBlock): string {
...
export function buildTeacherAttendanceCalcHtml(teacher: MockTeacherBlock, monthLabel: string): string {
      ...
      <table class="title-block" cellspacing="0" cellpadding="0">
        <tr>
          <td class="left">
            ...
            <p class="doc-sub">明學管理系統 · 行政處理用（原型）</p>
          </td>
...
export function teacherAttendancePdfFilename(teacher: MockTeacherBlock, monthLabel: string): string {
...
export async function downloadTeacherAttendancePdf(teacher: MockTeacherBlock): Promise<void> {
  const html = buildTeacherAttendanceCalcHtml(teacher, MOCK_MONTH_LABEL)
  const footer = `明學管理系統 · 老師出席計算頁（原型）· ${teacher.name} · ${MOCK_MONTH_LABEL}`
  const blob = await buildCalcPagePdfBlob(html, footer)
  downloadBlob(blob, teacherAttendancePdfFilename(teacher, MOCK_MONTH_LABEL))
}

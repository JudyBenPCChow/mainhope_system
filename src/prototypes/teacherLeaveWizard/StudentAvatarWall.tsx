...
import type { MockStudentKind, MockStudentOnLesson } from "./mockData"
import { InitialAvatar } from "./visualBits"

function studentDetail(student: MockStudentOnLesson): string {
  if (student.kind === "leave") {
    return `${student.student_code} · ${student.leaveReason ?? "請假"}／${student.leaveMakeup ?? "—"}`
  }
  if (student.kind === "makeup") {
    return `${student.student_code} · ${student.makeupFrom ?? "來補堂"}`
  }
  return student.student_code
}

function toneForKind(kind: MockStudentKind): "neutral" | "warning" | "info" | "success" {
  if (kind === "expected") return "warning"
  if (kind === "leave") return "neutral"
  return "info"
}

function StudentAvatar({ student }: { student: MockStudentOnLesson }) {
  return (
    <div
      className="flex flex-col items-center gap-1"
      title={studentDetail(student)}
    >
      <InitialAvatar name={student.full_name} size="md" tone={toneForKind(student.kind)} />
    ...
    >
      ...
      {students.length === 0 ? (
        ...
        <div className="flex flex-wrap gap-3">
          {students.map((st) => (
            <StudentAvatar key={st.id} student={st} />
          ))}
        </div>
      )}
    </div>
...
export function StudentAvatarWall({
  ...
  expected: MockStudentOnLesson[]
  leave: MockStudentOnLesson[]
  makeup: MockStudentOnLesson[]
}) {
  return (

...
/** 寫入收件匣事件；失敗時 throw（財務點名提醒等需要回饋）。 */
export async function recordInboxEventOrThrow(input: RecordInboxEventInput): Promise<void> {
...
export async function recordInboxEvent(input: RecordInboxEventInput): Promise<void> {
 if (!supabase) return
 try {
  await recordInboxEventOrThrow(input)
 } catch (e) {

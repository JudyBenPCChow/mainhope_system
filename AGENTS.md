# AGENTS.md — 明學（MainHope）行政後台專案總綱

本檔是 AI agent 的「入口指南」：先讀這份對齊全局，細節指向 `docs/`。介面用語一律 **繁體中文**；機構自稱見 `docs/TERMINOLOGY.md`（中文正式名 **明學教育**；英文品牌 **MainHope**，勿寫 mingxue；用校方／本校／補習社；禁院方、禁「明學補習社」、禁以書院／學院自稱）。
...
2. **側欄**：同步改 `src/lib/navStructure.ts` 的 `NAV_STRUCTURE`（`Layout.tsx` 只引用；含 `roles`：`admin`/`manager`/`teacher`/`alien`），避免「有路由沒入口」或相反。管理層入口體感見 `docs/backlog/mgmt-manager-role.md`。
...
- **角色**：目前前端角色為 `localStorage.mgmt_role`（`admin`/`manager`/`teacher`/`alien`，見 `src/lib/mgmtRole.ts`），**不等於** Supabase Auth，前端隱藏按鈕不代表有權限保護。管理層分流見 `docs/backlog/mgmt-manager-role.md`：**「少做日常」= 入口體感**（側欄偏決策／監督，非第二個行政台）；**讀權限 manager ≥ admin**（勿因隱藏入口而用 RLS 令 manager 讀少過 admin）；寫入／破壞性仍可窄過 admin。
...
**讀檔階梯**（慳 context）：預設只靠本檔。問 backlog → 只讀 `docs/BACKLOG.md`「進行中／未完成」表。做主題 → 只開該列 `docs/backlog/<topic>.md`（＋現行 `docs/plans/` 若有）。`docs/audits/`／已完成 `plans/` 除非對對抗、查決策、或使用者點名，否則唔開。下方係索引，**唔係**開局必讀清單。
...
- 文案與稱呼（明學教育／MainHope／校方／本校；禁院方、禁明學補習社、禁 mingxue 當英文品牌）：`docs/TERMINOLOGY.md`
- 正規／暑期學年與報讀：`docs/ACADEMIC_YEARS.md`（含 §1.1 後台寫入：不硬鎖歷史學年；非當期 confirm＋audit）
- 學費學期節奏與逾期罰款（規則已定；系統自動加罰尚未上線）：`docs/TUITION_TERM_AND_LATE_FEE_POLICY.md`；前線指引：`docs/manual/TUITION_LATE_FEE_FRONTLINE.md`
